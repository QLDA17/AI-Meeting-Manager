...
import { Search, History, Filter, Download, ShieldAlert, User, Building2, Cpu } from 'lucide-react';
...
const AdminAuditLogs: React.FC = () => {
  ...
  const getActionColor = (action: string) => {
    const act = action || '';
    if (act.includes('UPDATE') || act.includes('CREATE')) return 'text-blue-600 bg-blue-50 dark:bg-blue-900/20';
    if (act.includes('DELETE') || act.includes('SUSPEND')) return 'text-red-600 bg-red-50 dark:bg-red-900/20';
    if (act.includes('UPLOAD')) return 'text-green-600 bg-green-50 dark:bg-green-900/20';
