...
import type { User, Session, SystemRole, OrgUser, GroupUser } from '@/shared/types';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = useCallback(async (username: string, password: string) => {
    ...
    try {
      ...
      if (status === 401) {
        setError(detail || 'Tên đăng nhập hoặc mật khẩu không đúng.');
      } else if (status === 422) {
        setError('Dữ liệu đăng nhập không hợp lệ.');
      } else {
        // MOCK FALLBACK: Bỏ qua lỗi server và đăng nhập bằng mockData
        console.warn('Backend unavailable, falling back to mock data...');
        const mockUser = mockUsers.find(u => u.email === username);
        if (mockUser) {
          setUser(mockUser);
          setSession({
            userId: mockUser.id,
            currentOrgId: mockUser.orgMemberships?.[0]?.orgId || '',
            token: 'mock-jwt-token-for-frontend-only',
            expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000),
            createdAt: new Date(),
          });
          return;
