...
def test_org_admin_can_fetch_scoped_audit_logs(client: TestClient, db_session: Session):
    org_admin = make_user(db_session, "orgaudit", "orgaudit@example.com")
    outsider = make_user(db_session, "outsider", "outsider@example.com")
    org = make_active_org(db_session, org_admin, name="Audit Org")
    other_org = create_organization(db_session, {"name": "Other Org", "settings": {"approval_status": "active"}})
    add_user_to_organization(db_session, outsider.id, other_org.id, "member")

    db_session.add_all(
        [
            models.AuditLog(
                user=org_admin.username,
                role="org-admin",
    ...
    response = client.get(
        ...
        headers=auth_headers(client, org_admin.username),
    )

    assert response.status_code == 200
    payload = response.json()
    assert len(payload) == 1
    assert payload[0]["org"] == org.name
    assert payload[0]["action"] == "UPDATE_ORGANIZATION"


def test_non_org_admin_cannot_fetch_org_audit_logs(client: TestClient, db_session: Session):
    org_admin = make_user(db_session, "orgowner", "orgowner@example.com")
    member = make_user(db_session, "memberaudit", "memberaudit@example.com")
    org = make_active_org(db_session, org_admin, name="Private Audit Org")
