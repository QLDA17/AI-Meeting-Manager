...
from src.api.core.admin_runtime import get_admin_settings_snapshot, update_admin_settings_values
...
def test_register_public_disabled(client, db_session):
    original = get_admin_settings_snapshot(db_session)["public_registration_enabled"]
    update_admin_settings_values({"public_registration_enabled": False}, db_session)
    try:
        response = client.post("/api/auth/register", json=register_payload())
        ...
        update_admin_settings_values({"public_registration_enabled": original}, db_session)
